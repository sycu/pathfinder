...
class AStar(Algorithm):
    ...
    def solve(self, graph: Graph, source: Vertex, target: Vertex) -> Generator[Vertex, None, Tuple[List[Vertex], float]]:
        ...
        while open_set:
            ...
            if vertex == target:
                return self.__build_path(sources, source, target), g_score[target]

            for edge_target, edge_cost in graph.edges[vertex].items():
                tentative_g_score = g_score[vertex] + edge_cost
                if tentative_g_score < g_score[edge_target]:
                    sources[edge_target] = vertex
                    g_score[edge_target] = tentative_g_score
                    f_score[edge_target] = g_score[edge_target] + self.heuristic.calculate(edge_target, target)
